...
async function fixStatus() {
  await supabase.from('telegram_profiles').update({ status: 'Active' }).eq('chat_id', '7527336375');
  await supabase.from('telegram_profiles').update({ status: 'Active' }).eq('chat_id', '347835367');
